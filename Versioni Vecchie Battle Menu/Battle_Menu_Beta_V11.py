...
def play_background_music(music_path, volume=0.7, loops=-1):
    """
...
# Music file path
BACKGROUND_MUSIC_PATH = r"C:\Users\franc\Afterdeath-RPG\Musics\1-33 Gym Leader Battle!.mp3"
...
def apply_move_effects(move, target_character, target_part_index=None):
    ...
    available_effects = ['burn', 'bleeding', 'poison', 'stun', 'confusion']
...
def display_character_moves(character):
    ...
    for i, move in enumerate(character.moves, 1):
        ...
        if move.eff_appl:
            # Handle both old format (strings) and new format (lists)
            effect_strings = []
            for effect in move.eff_appl:
                if isinstance(effect, str):
                    effect_strings.append(effect)
                elif isinstance(effect, list) and len(effect) >= 4:
                    # Format: [name, level, duration, immunity]
                    effect_strings.append(f"{effect[0]}(L{effect[1]},D{effect[2]})")
                else:
                    effect_strings.append(str(effect))
            print(f"   Effects: {', '.join(effect_strings)}")
        if move.reqs:
...
def initialize_game_data():
    ...
    apply_buff_debuff(player, 'forz', 1)  # Example buff to player's forz stat (will be scaled by 2.0)
    apply_buff_debuff(player, 'vel', 1)   # Example buff to player's vel stat (will be scaled by 1.0)
    ...
    add_move_to_character(player, "Fendente", "ATK", 1.2, 0.8, 0.0, 
                         effects=["Bleeding", 1, 1, 0], requirements=["SPADA"], 
