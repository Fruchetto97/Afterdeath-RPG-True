...
def update_character_stats(character):
    ...
    for stat_name, (max_attr, current_attr, buf_attr) in stat_mappings.items():
        if hasattr(character, max_attr) and hasattr(character, buf_attr) and hasattr(character, current_attr):
            ...
            # Calculate new current value: max + (buff * scaling_factor)
            new_current = max_value + (buf_value * scaling_factor)
            
            # Ensure the value doesn't go below 0
            new_current = max(0, new_current)
            
            setattr(character, current_attr, new_current)
...
class Character:
    def __init__(self, name, max_pvt, pvt, max_rig, rig, buf_rig, max_res, res, buf_res, max_sta, sta, buf_sta,
                  ...
                  body_parts, image_path, moves=None):
        self.name = name
...
player = Character(
    ...
    player_body_parts,
    str(player_image_path)  # Use relative path for portability
)
...
enemy = Character(
    ...
    100, 80, 20, 20,0, 50, 40,0, 12, 12,0, 15, 15, 0,14, 14,0, 13, 13,0, 11, 11,0,
    enemy_body_parts,
    str(enemy_image_path)  # Use relative path for portability
)
...
body_panel_width = 180  # Adjust as needed
body_panel_height = image_size + 50  # Adjust as needed
...
for idx, part in enumerate(player.body_parts):
    tk.Label(body_panel, text=part.name, fg="white", bg="black", font=("Times", 16), anchor="w").place(x=10, y=10 + idx*40)
...
def periodic_stat_update():
    ...
    if 'sta' in stat_bars:
        update_stat_bar(stat_bars['sta'], player.sta, player.max_sta)
    ...
    if menu_selection_index == 0:
        ...
        try:
            ...
            if frames:
                ...
                tk.Label(enemy_image_frame, text="No Image", fg="white", bg="black").place(relx=0.5, rely=0.5, anchor="center")
        except Exception as e:
            tk.Label(enemy_image_frame, text="No Image", fg="white", bg="black").place(relx=0.5, rely=0.5, anchor="center")
...
def move_selection(event):
    ...
    # Clear any existing up/down bindings before setting new ones
    root.unbind("<Up>")
    root.unbind("<Down>")
...
def show_moves_panel():
    global moves_table_frame, moves_selection_index, moves_triangle_cursor
    ...
    if menu_selection_index == 1:
        ...
        available_width = 1600 - LEFT_PANEL_WIDTH - 20  # 1285 pixels
        # Available height: from top to menu bar, minus character name area
        available_height = MENU_TOP_Y + 240 - 50  # 730 pixels
        ...
        moves_table_frame.place(x=LEFT_PANEL_WIDTH + 70, y=50)  # Moved 40 pixels right to allow arrow inside
        ...
        content_height = available_height - 100  # Space for title and padding
            ...
            for row_idx, move in enumerate(player.moves):
                ...
                for col_idx, (data, width_px) in enumerate(zip(move_data, col_widths_pixels)):
                    ...
                    cell_label = tk.Label(
                        ...
                        fg=color,
                        bg=bg_color,
                        font=("Times", 10),  # Smaller font to fit more columns
                        width=width_px//7,  # Convert pixels to character width
                        height=2,  # Cell height
                        anchor="center",
                        relief="solid",
                        borderwidth=1,
                        wraplength=width_px-5  # Wrap text based on pixel width
                    )
        ...
        def draw_moves_triangle_cursor():
            ...
            if hasattr(player, 'moves') and player.moves and len(player.moves) > 0:
                ...
                cursor_y = 175 + (current_index * row_height)
                ...
                print(f"DEBUG CALCULATION: 173 + ({current_index} * {row_height}) = {173 + (current_index * row_height)}")
        ...
        # Handler for up/down navigation in moves menu
        def on_moves_nav(event):
            ...
            # Clear and update selection index
            ...
            # Debug navigation change
            print(f"NAVIGATION: {event.keysym} pressed, index changed from {old_index} to {moves_selection_index}")
            
            # Force redraw with updated index
            draw_moves_triangle_cursor()
        
        # Bind up/down keys for moves navigation
