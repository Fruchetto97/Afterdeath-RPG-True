...
def show_target_selection():
    ...
    target_selection_frame.place(x=LEFT_PANEL_WIDTH + 50, y=100)
    target_selection_frame.propagate(False)
    
    # Title
    title_label = tk.Label(
        target_selection_frame,
        text=f"Seleziona il bersaglio per: {selected_move.name}",
        fg="red",
        bg="black",
        font=("Times", 20, "bold")
    )
    ...
    parts_frame.pack(pady=20)
    
    for idx, part in enumerate(enemy.body_parts):
        part_label = tk.Label(
            parts_frame,
            text=f"{part.name} ({part.p_pvt}/{part.max_p_pvt} PVT)",
            fg="white",
            bg="black",
            font=("Times", 16),
            anchor="w",
        ...
        part_label.pack(pady=3, padx=50, anchor="w")
    ...
    def draw_target_triangle():
        ...
        triangle_size = 16
        cursor_x = LEFT_PANEL_WIDTH + 100
        cursor_y = 220 + (target_selection_index * 28)
    ...
    def on_target_nav(event):
        ...
        if event.keysym == "Down":
            ...
            execute_move()
            return
        elif event.keysym == "Escape":  # ESC key to cancel
            cancel_move_usage()
            return
...
def cancel_move_usage():
    """Cancel move usage and return to moves menu"""
...
def setup_moves_navigation():
    """Set up navigation bindings for moves menu"""
    def on_moves_nav(event):
        ...
        print(f"NAVIGAZIONE: {event.keysym} premuto, indice cambiato da {old_index} a {moves_selection_index}")
        
        # Force redraw with updated index
        draw_moves_triangle_cursor()
    
    # Bind navigation keys for moves
