...
def calculate_move_damage(character, strength_scaling, dexterity_scaling, special_scaling):
    """
    Calculate the damage of a move based on character stats and scaling factors.
    
    Formula: Damage = Strength_scaling * Current_strength + Dexterity_scaling * Current_dexterity + Special_scaling * Current_special
    ...
    Returns:
        int: Calculated damage value (rounded)
    """
...
def calculate_move_stamina_cost(strength_scaling, dexterity_scaling, special_scaling, effects=None, requirements=None):
    """
    Calculate the stamina cost of a move based on scaling factors, effects, and requirements.
    
    Formula: Stamina_cost = 2 + (sum of all scaling values) + (number of effects * eff_stm_sca_factor) - (number of requirements * req_stm_sca_factor)
    
    Args:
        strength_scaling (float): Strength scaling factor
        dexterity_scaling (float): Dexterity scaling factor
        special_scaling (float): Special scaling factor
        effects (list): List of effects (optional)
        requirements (list): List of requirements (optional)
        
    Returns:
        int: Calculated stamina cost (rounded, minimum 1)
    """
    # Base cost
    base_cost = 2
...
class Character:
    def __init__(self, name, max_pvt, pvt, max_rig, rig, buf_rig, max_res, res, buf_res, max_sta, sta, buf_sta,
        ...
        self.name = name
        self.max_pvt = max_pvt  # MAX STATS
...
class Ability:
    def __init__(self, name, punti, description, descriptionLong):
        self.name = name
        self.punti = punti  # ATK, BUF, REA
        self.description = description  # Strength scaling
        self.descriptionLong = descriptionLong  # Dexterity scaling  
...
player_image_path = Path(__file__).parent / "Characters_PNGs" / "Bremman_NPC_01.png"

# Example enemy with custom body parts and animated gif

enemy_image_path = Path(__file__).parent / "Enemies_GIFs" / "Blubbertone.gif"
...
# Check if character already has a move with the same name
    if hasattr(character, 'ability_name'):
        for existing_ability_name in character.ability_name:
            if existing_ability_name.name == ability_name:
                print(f"Character {getattr(character, 'name', 'Unknown')} already has a move named '{ability_name}'")
                return False
...
def initialize_game_data():
    ...
    add_ability_to_character(player, "Chanche!", 1, ["se CONFUSE/STUN,  +1 STR"], 
...
# Bottom menu
menu_labels = ["NEMICO", "MOSSE", "EFFETTI", "ABILITÀ", "OGGETTI"]
...
menu_label_widgets = []
menu_spacing = [10, 180, 320, 475, 633]  # Same as MENU_ITEM_SPACING

for idx, label in enumerate(menu_labels):
    lbl = tk.Label(menu_bar, text=label, fg="white", bg="black", font=("Times", 18))
...
def periodic_stat_update():
    ...
    This ensures stats stay synchronized with buffs/debuffs.
    """
...
TRIANGLE_SIZE = 24
MENU_ITEM_SPACING = [10, 180, 320, 475, 633]  # Spacing between menu items
...
def show_enemy_panel():
    ...
    if menu_selection_index == 0:
        ...
        parts_x = LEFT_PANEL_WIDTH + 740
        parts_y = MENU_TOP_Y - 520 + 40
        enemy_parts_frame.place(x=parts_x, y=parts_y)
        ...
        def draw_enemy_parts_triangle():
            ...
            enemy_parts_triangle.place(x=parts_x + 2, y=parts_y + y_offset)
        ...
        def update_enemy_part_display():
            draw_enemy_parts_triangle()
            show_enemy_part_attributes()

        update_enemy_part_display()

        # Handler for up/down navigation in enemy parts menu
        ...
        # Bind up/down keys for enemy navigation
...
def show_target_selection():
    ...
    instructions = tk.Label(
        target_selection_frame,
        # btext="Usa ↑↓ per selezionare, ENTER per confermare, ESC per annullare",
        fg="gray",
        bg="black",
        font=("Times", 14)
    )
...
def execute_move():
    ...
    if accuracy_roll > selected_move.accuracy:
        ...
        print(f"Attack hit! {hit_message}")
        
        # Apply damage to target part
        target_part.p_pvt -= selected_move.danno
        if target_part.p_pvt < 0:
            target_part.p_pvt = 0
        
        print(f"Damage dealt: {selected_move.danno}. Target part {target_part.name} PVT: {target_part.p_pvt}/{target_part.max_p_pvt}")
...
def setup_ability_navigation():
    def on_ability_nav(event):
        ...
        if menu_selection_index != 1:  # Only work in MOSSE menu
            return
        if ability_usage_active:  # Don't interfere with target selection
            return
        ...
        if event.keysym == "Down":
            ...
            highlight_selected_ability_row()  # Highlight on ENTER
            see_selected_ability()
            return
        
        #  draw_ability_triangle_cursor()
...
def see_selected_ability():
    """Attempt to use the currently selected ability"""
    global ability_usage_active, selected_ability
    
    if menu_selection_index != 1:  # Not in ability menu
        return
...
def show_ability_description():
    """Show ability description"""
    global ability_selection_frame, ability_selection_index
    
    # Clear any existing target selection
    if ability_selection_frame is not None:
        ability_selection_frame.destroy()
        ability_selection_frame = None
    if ability_selection_frame is not None:
        ability_selection_frame.destroy()
        ability_selection_frame = None
    
    ability_selection_index = 0
    
    # Create target selection frame (similar to enemy panel but for targeting)
    ability_selection_frame = tk.Frame(
        root,
        bg="black",
        highlightbackground="white",
        highlightthickness=2,
    ...
    ability_selection_frame.place(x=LEFT_PANEL_WIDTH + 50, y=15)
    ability_selection_frame.propagate(False)

    ability_info = (
        f"{selected_ability.name}\n"
        f"Punti: {selected_ability.punti} | "
        f"Descrizione: {selected_ability.descriptionLong}"
    )
    ability_info_label = tk.Label(
        ability_selection_frame,
        text=ability_info,
        fg="#FFFFFF",
        bg="black",
        font=("Times", 16, "bold"),
    ...
    ability_info_label.pack(pady=5)
...
def highlight_selected_ability_row():
    global ability_row_frames, mability_selection_index
...
def show_moves_panel():
    ...
    if menu_selection_index == 1:
        ...
        # Draw move selection triangle cursor if there are moves
        # draw_moves_triangle_cursor()
        
        # Set up moves navigation
        setup_moves_navigation()

        # After creating all rows in show_moves_panel()
        highlight_selected_move_row()
        ...
        if hasattr(player, 'ability') and player.ability:
            ...
            for row_idx, ability in enumerate(player.ability):
                ...
                # Prepare individual effects, requirements, and elements
                effects = ability.eff_appl if ability.eff_appl else []
                requirements = ability.reqs if ability.reqs else []
                elements = ability.elem if ability.elem else []
                ...
                ability_data = [
                    ability.name,                           # NOME
                    ability.punti,                          # PUNTI
                    ability.descrizione,               # DESCRIZIONE
                ]
            ...
            no_ability_label = tk.Label(
                ...
                fg="gray",
                bg="black",
                font=("Times", 18),  # Adjusted font for no ability message
                pady=40  # Padding
            )
        ...
        # Draw ability selection triangle cursor if there are ability
        # draw_ability_triangle_cursor()
        
        # Set up ability navigation
        setup_ability_navigation()

        # After creating all rows in show_ability_panel()
        highlight_selected_ability_row()

    elif menu_selection_index == 4:
        available_width = 1000  # Set a fixed width for the panel
        available_height = MENU_TOP_Y + 240 - 150  # 730 pixels
        ...
        instructions_label = tk.Label(
            moves_table_frame,
            text="Usa ↑↓ per navigare, ENTER per dettagli oggetti",
            fg="white",
            bg="black",
            font=("Times", 14)
        )
...
def move_selection(event):
    ...
    # Clear any existing up/down/enter bindings before setting new ones
    root.unbind("<Up>")
    root.unbind("<Down>")
    root.unbind("<Return>")
    root.unbind("<Escape>")
    ...
    show_moves_panel()  # Update moves panel based on selection
...
# Show enemy panel at startup if needed
show_enemy_panel()
show_moves_panel()
