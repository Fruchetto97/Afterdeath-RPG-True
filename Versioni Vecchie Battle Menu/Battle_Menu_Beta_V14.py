...
# Music file path
BACKGROUND_MUSIC_PATH = r"C:\Users\franc\Desktop\Afterdeath_RPG\Musics\Battle-Walzer.MP3"
...
def debug_simulate_player_turn():
    """Debug function to simulate a player turn"""
    print("DEBUG: Simulating player turn")
    result = end_player_turn()
    print(f"DEBUG: Player turn simulation complete, returned Turn: {result}")
    return result

def debug_simulate_enemy_turn():
    """Debug function to simulate an enemy turn"""
    print("DEBUG: Simulating enemy turn")
    result = end_enemy_turn()
    print(f"DEBUG: Enemy turn simulation complete, returned Turn: {result}")
    return result

def debug_simulate_full_round():
    """Debug function to simulate a full round (both player and enemy turns)"""
    print("DEBUG: Simulating full round")
    player_result = debug_simulate_player_turn()
    enemy_result = debug_simulate_enemy_turn()
    print(f"DEBUG: Full round complete. Final Turn: {Turn}")
    return Turn
...
# Global variables for body part HP bars
...
for idx, part in enumerate(player.body_parts):
    ...
    hp_canvas.create_rectangle(15, 0, body_part_bar_width - 15, body_part_bar_height, fill="#222", outline="")
    fill_width = int((body_part_bar_width - 30) * part.p_pvt / part.max_p_pvt) if part.max_p_pvt > 0 else 0
    hp_canvas.create_rectangle(15, 0, 15 + fill_width, body_part_bar_height, fill="green", outline="")
...
# Turn display (above character name)
turn_display_frame = tk.Frame(
    root,
    bg="black",
...
turn_display_frame.grid(row=8, column=0, padx=0, pady=0)
turn_display_frame.grid_propagate(False)  # Prevent frame from resizing to content

# Global variables for turn display labels
turn_info_label = tk.Label(turn_display_frame, width=22, text="Turn: 0 | P: 0 | E: 0", fg="yellow", bg="black", font=("Times", 12))
turn_info_label.pack(expand=True, fill="both")

def update_turn_display():
    """Update the turn display with current turn information"""
    global turn_info_label
    turn_text = f"Turn: {Turn} | P: {turn_player} | E: {turn_enemy}"
    turn_info_label.config(text=turn_text)
    print(f"DEBUG: Updated turn display to: {turn_text}")
...
try:
    # Load animated GIF with fixed zoom and crop transformations
    img = Image.open(player.image_path)
    frames = []
    
    print(f"DEBUG: Loading player image from: {player.image_path}")
    print(f"DEBUG: Player image format: {img.format}, mode: {img.mode}, size: {img.size}")
    
    try:
        while True:
            frame = img.copy().convert("P", palette=Image.ADAPTIVE, colors=256)
            original_width, original_height = frame.size
            
            # Apply fixed 400% zoom (4x scale) maintaining aspect ratio
            zoom_factor = 10.0
            zoomed_width = int(original_width * zoom_factor)
            zoomed_height = int(original_height * zoom_factor)
            frame = frame.resize((zoomed_width, zoomed_height), Image.LANCZOS)
            
            # Crop vertically by 0.4 (keep top 60% to focus on head)
            crop_ratio = 0.45  # Keep top 60%
            crop_height = int(zoomed_height * crop_ratio)
            frame = frame.crop((0, 0, zoomed_width, crop_height))
            
            # Final resize to fit the UI frame while maintaining aspect ratio
            target_size = image_size +400
            
            # Calculate aspect ratio of cropped image
            crop_aspect_ratio = zoomed_width / crop_height
            
            # Determine final dimensions to fit within target_size while maintaining aspect ratio
            if crop_aspect_ratio > 1:  # Wider than tall
                final_width = target_size
                final_height = int(target_size / crop_aspect_ratio)
            else:  # Taller than wide or square
                final_height = target_size
                final_width = int(target_size * crop_aspect_ratio)
    ...
    print(f"Successfully loaded {len(frames)} frames from player GIF with fixed transformations")
    
    if frames:
        img_label = tk.Label(image_frame, bg="black")
        img_label.place(relx=0.5, rely=0.5, anchor="center")
        
        def animate_player(index=0):
            img_label.config(image=frames[index])
            img_label.image = frames[index]
            image_frame.after(img.info.get('duration', 100), animate_player, (index + 1) % len(frames))
        
        print("DEBUG: Starting player GIF animation with fixed zoom and crop")
        animate_player()
    else:
        tk.Label(image_frame, text="No Frames", fg="red", bg="black").place(relx=0.5, rely=0.5, anchor="center")
        
except Exception as e:
    print(f"Error loading player image: {e}")
    print(f"Attempted to load: {player.image_path}")
...
def update_body_part_hp_bars():
    ...
    global body_part_hp_canvases, body_part_bar_width, body_part_bar_height
    
    print(f"DEBUG: update_body_part_hp_bars called - {len(body_part_hp_canvases)} canvases, {len(player.body_parts)} body parts")
    ...
    if len(body_part_hp_canvases) == len(player.body_parts):
        for idx, (canvas, part) in enumerate(zip(body_part_hp_canvases, player.body_parts)):
            try:
                print(f"DEBUG: Updating part {idx} ({part.name}): {part.p_pvt}/{part.max_p_pvt}")
                canvas.delete("all")
                canvas.create_rectangle(15, 0, body_part_bar_width - 15, body_part_bar_height, fill="#222", outline="")
                fill_width = int((body_part_bar_width - 30) * part.p_pvt / part.max_p_pvt) if part.max_p_pvt > 0 else 0
                canvas.create_rectangle(15, 0, 15 + fill_width, body_part_bar_height, fill="green", outline="")
        ...
        print(f"Mismatch: {len(body_part_hp_canvases)} canvases vs {len(player.body_parts)} body parts")
...
right_stat_labels['forz'] = draw_right_stat_row(stats_right_frame, "FOR", player.forz, player.max_for, get_character_buff_level(player, 'forz'))
right_stat_labels['des'] = draw_right_stat_row(stats_right_frame, "DES", player.des, player.max_des, get_character_buff_level(player, 'des'))
right_stat_labels['spe'] = draw_right_stat_row(stats_right_frame, "SPE", player.spe, player.max_spe, get_character_buff_level(player, 'spe'))
right_stat_labels['vel'] = draw_right_stat_row(stats_right_frame, "VEL", player.vel, player.max_vel, get_character_buff_level(player, 'vel'))
...
def periodic_stat_update():
    ...
    # Store RES value before updates for debugging
    res_before = player.res
    ...
    # Check if RES was changed by stat updates
    if res_before != player.res and move_recalc_counter % 10 == 0:
        print(f"DEBUG: periodic_stat_update changed RES from {res_before} to {player.res}")
    ...
    if 'rig' in stat_bars:
        # Use effective_max_rig if available, otherwise fall back to max_rig
        max_rig_display = getattr(player, 'effective_max_rig', player.max_rig)
        update_stat_bar(stat_bars['rig'], player.rig, max_rig_display)
    if 'res' in stat_bars:
        # Use effective_max_res if available, otherwise fall back to max_res
        max_res_display = getattr(player, 'effective_max_res', player.max_res)
        update_stat_bar(stat_bars['res'], player.res, max_res_display)
        # Debug output every 50 cycles (5 seconds) to avoid spam
        if move_recalc_counter % 50 == 0:
            print(f"DEBUG: RES stat bar update - current: {player.res}, max_display: {max_res_display}")
    if 'sta' in stat_bars:
    ...
    if 'forz' in right_stat_labels:
        ...
        labels['buf'].config(text=str(get_character_buff_level(player, 'buf_forz')))
    ...
    if 'des' in right_stat_labels:
        ...
        labels['buf'].config(text=str(get_character_buff_level(player, 'buf_des')))
    ...
    if 'spe' in right_stat_labels:
        ...
        labels['buf'].config(text=str(get_character_buff_level(player, 'buf_spe')))
    ...
    if 'vel' in right_stat_labels:
        ...
        labels['buf'].config(text=str(get_character_buff_level(player, 'buf_vel')))
...
def show_enemy_panel():
    ...
    if menu_selection_index == 0:
        ...
        # Set up enemy navigation when panel is shown
...
regeneration_mode_active = False  # Flag to indicate if regeneration mode is active
regeneration_selection_index = 0  # Current body part selection for regeneration

def regenerate_body_part(character, part_index):
    """
    Regenerate a specific body part if the character has enough resources.
    ...
    # Check if the body part is already at full health
    if part.p_pvt >= part.max_p_pvt:
        show_message("Questa parte del corpo è già completamente guarita!", 1500)
        return False
    
    # Check regeneration (rig) resource
    if character.rig < 5:
        show_message("Non puoi più rigenerarti!", 1500)
        return False
    
    # Check stamina resource
    if character.sta < 1:
        show_message("Non hai abbastanza stamina!", 1500)
        return False
    
    # Warning if player has low RES (won't be able to refill RIG next turn)
    if hasattr(character, 'res') and character.res < 5:
        show_message("ATTENZIONE: RES insufficiente per ripristinare RIG nel prossimo turno!", 1500)
        # Still allow regeneration but warn the player
    ...
    update_body_part_hp_bars()
    if 'pvt' in stat_bars:
        update_stat_bar(stat_bars['pvt'], character.pvt, character.max_pvt)
    
    # Show success message
    show_message(f"Rigenerato {part.name}: +{actual_healing} HP\nRIG: -{5}, STA: -{1}", 2000)
    
    print(f"Regenerated {character.name}'s {part.name}: {old_pvt} -> {part.p_pvt} (+{actual_healing})")
    print(f"Resources consumed - RIG: {character.rig}, STA: {character.sta}")
    
    return True

def update_body_part_highlighting():
    """Update the highlighting of body parts based on regeneration selection"""
    global regeneration_selection_index, body_part_hp_canvases
    
    if not regeneration_mode_active:
        return
        
    # Reset all body part labels to white
    for idx, part in enumerate(player.body_parts):
        # Find the label widget for this body part
        for widget in body_panel.winfo_children():
            if isinstance(widget, tk.Label) and widget.cget("text") == part.name:
                if idx == regeneration_selection_index:
                    widget.config(fg="green", font=("Times", 16, "bold"))  # Highlight selected
                else:
                    widget.config(fg="white", font=("Times", 16))  # Normal
                break

def toggle_regeneration_mode():
    """Toggle regeneration mode on/off"""
    global regeneration_mode_active, regeneration_selection_index
    
    if not player_has_control:  # Don't allow during enemy turn
        return
    
    regeneration_mode_active = not regeneration_mode_active
    regeneration_selection_index = 0  # Reset selection
    
    if regeneration_mode_active:
        print("Regeneration mode activated")
        show_message("Modalità Rigenerazione Attivata\nUsa ↑↓ per selezionare, SPACEBAR per rigenerare, R per uscire", 2500)
        
        # Clear current menu navigation
        root.unbind("<Up>")
        root.unbind("<Down>")
        root.unbind("<space>")
        root.unbind("<Escape>")
        
        # Set up regeneration navigation
        setup_regeneration_navigation()
        
        # Update highlighting
        update_body_part_highlighting()
    else:
        print("Regeneration mode deactivated")
        show_message("Modalità Rigenerazione Disattivata", 1000)
...
def setup_regeneration_navigation():
    """Set up navigation for regeneration mode"""
    def on_regeneration_nav(event):
        global regeneration_selection_index
        
        if not regeneration_mode_active:
            return
            
        if event.keysym == "Down":
            regeneration_selection_index = (regeneration_selection_index + 1) % len(player.body_parts)
        elif event.keysym == "Up":
            regeneration_selection_index = (regeneration_selection_index - 1) % len(player.body_parts)
        elif event.keysym == "space":  # SPACEBAR key
            regenerate_body_part(player, regeneration_selection_index)
            return
        
        # Update highlighting
        update_body_part_highlighting()
    
    root.bind("<Up>", on_regeneration_nav)
    root.bind("<Down>", on_regeneration_nav)
    root.bind("<space>", on_regeneration_nav)

# --- CHARACTER GIF BLINK SYSTEM ---

def blink_character_gif(character_type, blink_color="red", blink_duration=300, blink_count=3):
    """
    Make a character's GIF blink by overlaying a colored rectangle
    
    Args:
        character_type (str): "player" or "enemy" to specify which character
        blink_color (str): Color of the blink overlay ("red", "white", "yellow", etc.)
        blink_duration (int): Duration of each blink in milliseconds
        blink_count (int): Number of blinks
    """
    global player_blink_overlay, enemy_blink_overlay
    
    # Initialize blink overlay variables if they don't exist
    if 'player_blink_overlay' not in globals():
        global player_blink_overlay
        player_blink_overlay = None
    if 'enemy_blink_overlay' not in globals():
        global enemy_blink_overlay
        enemy_blink_overlay = None
    
    def create_blink_overlay(target_frame, overlay_var_name):
        """Create a blink overlay on the specified frame"""
        # Remove existing overlay if present
        overlay = globals().get(overlay_var_name)
        ...
        # Create new overlay
        overlay = tk.Frame(target_frame, bg=blink_color, width=target_frame.winfo_width(), height=target_frame.winfo_height())
        overlay.place(x=0, y=0, relwidth=1, relheight=1)
        overlay.lift()  # Bring to front
        
        # Store reference
        globals()[overlay_var_name] = overlay
        return overlay
    
    def remove_blink_overlay(overlay_var_name):
        """Remove the blink overlay"""
        overlay = globals().get(overlay_var_name)
        if overlay is not None:
            try:
                overlay.destroy()
                ...
                pass
            globals()[overlay_var_name] = None
    
    def blink_sequence(remaining_blinks):
        """Recursive function to handle the blinking sequence"""
        if remaining_blinks <= 0:
            return
        
        # Determine target frame and overlay variable name
        if character_type.lower() == "player":
            target_frame = image_frame
            overlay_var_name = "player_blink_overlay"
        elif character_type.lower() == "enemy":
            if enemy_image_frame is not None:
                target_frame = enemy_image_frame
                overlay_var_name = "enemy_blink_overlay"
            else:
                print(f"Warning: Enemy image frame not available for blinking")
                return
        else:
            print(f"Warning: Unknown character type '{character_type}' for blinking")
            return
        
        # Create blink overlay (show)
        create_blink_overlay(target_frame, overlay_var_name)
        
        # Schedule removal of overlay (hide) after half the blink duration
        def hide_blink():
            remove_blink_overlay(overlay_var_name)
            
            # Schedule next blink if more remain
            if remaining_blinks > 1:
                root.after(blink_duration // 2, lambda: blink_sequence(remaining_blinks - 1))
        
        root.after(blink_duration // 2, hide_blink)
    
    # Start the blinking sequence
    blink_sequence(blink_count)

def blink_player(blink_color="red", blink_duration=100, blink_count=3):
    """Convenience function to blink the player character"""
    blink_character_gif("player", blink_color, blink_duration, blink_count)

def blink_enemy(blink_color="red", blink_duration=300, blink_count=3):
    """Convenience function to blink the enemy character"""
    blink_character_gif("enemy", blink_color, blink_duration, blink_count)

# Global variables for blink overlays
player_blink_overlay = None
enemy_blink_overlay = None
...
def show_message(text, duration=1000):
    ...
    # Position the message at the bottom of the screen (over stat menu area)
    message_label.place(relx=0.5, rely=0.85, anchor="center")
    
    # Ensure the message appears on the topmost layer
    message_label.lift()
    message_label.tkraise()
...
def show_target_selection():
    ...
    effects_display = []
    if selected_move.eff_appl:
        for effect_data in selected_move.eff_appl:
            if isinstance(effect_data, str):
                # Old format: just effect name
                ...
                level = effect_data[1]
                duration = effect_data[2]
                effects_display.append(f"{effect_name} (Lv.{level}, {duration} turni)")
            else:
    ...
    attack_info = (
        ...
        f"Effetti: {', '.join(effects_display) if effects_display else 'Nessuno'}\n"
    ...
    instructions = tk.Label(
        target_selection_frame,
        text="Usa ↑↓ per selezionare, SPACEBAR per confermare, ESC per annullare",
        fg="gray",
        bg="black",
        font=("Times", 14)
    )
...
def execute_move():
    ...
    if accuracy_roll > selected_move.accuracy:
        ...
        apply_move_effects(selected_move, enemy, target_selection_index)
...
def show_abilities_panel():
    ...
    if menu_selection_index == 3:
        ...
        instructions_label = tk.Label(
            ability_table_frame,
            text="Usa ↑↓ per navigare, SPACEBAR per dettagli abilità",
            fg="white",
            bg="black",
            font=("Times", 14)
        )
...
def setup_moves_navigation():
    def on_moves_nav(event):
        ...
        if event.keysym == "Down":
            ...
            moves_selection_index = (moves_selection_index - 1) % len(player.moves)
        elif event.keysym == "space":  # SPACEBAR key to use move
            highlight_selected_move_row()  # Highlight on SPACEBAR
            use_selected_move()
            return
...
def setup_ability_navigation():
    def on_ability_nav(event):
        ...
        if event.keysym == "Down":
            ...
            ability_selection_index = (ability_selection_index - 1) % len(player.ability)
        elif event.keysym == "space":  # SPACEBAR key to see ability details
    ...
    root.bind("<space>", on_ability_nav)
...
def draw_moves_triangle_cursor():
    ...
    # Force update display before continuing
    root.update_idletasks()
...
def show_moves_panel():
    ...
    if menu_selection_index == 1:
        ...
        instructions_label = tk.Label(
            moves_table_frame,
            text="Usa ↑↓ per navigare, SPACEBAR per usare la mossa",
            fg="white",
            bg="black",
            font=("Times", 14)
        )
        ...
        if hasattr(player, 'moves') and player.moves:
            ...
            for row_idx, move in enumerate(player.moves):
                ...
                # Format effects with proper level/duration/immunity display
                formatted_effects = []
                for effect in effects:
                    if isinstance(effect, str):
                        ...
                        # Format: [name, level, duration, immunity]
                        formatted_effects.append(f"{effect[0]}\nL{effect[1]} D{effect[2]}")
                    else:
                        formatted_effects.append(str(effect))
                
                # Pad lists to ensure we have 4 effects, 4 requirements, and 2 elements
                effects_padded = (formatted_effects + ["-"] * 4)[:4]
        ...
        moves_table_frame.place(x=center_x + 150, y=75)  # Centered
        moves_table_frame.propagate(False)
        ...
        instructions_label = tk.Label(
            moves_table_frame,
            text="Usa ↑↓ per navigare, SPACEBAR per dettagli effetti",
            fg="white",
            bg="black",
            font=("Times", 14)
        )
        ...
        instructions_label = tk.Label(
            moves_table_frame,
            text="Usa ↑↓ per navigare, SPACEBAR per usare oggetto",
            fg="white",
            bg="black",
            font=("Times", 14)
        )
...
def setup_passa_navigation():
    ...
    root.bind("<space>", on_passa_nav)

def start_enemy_turn():
    """Start the enemy turn sequence"""
    global enemy_turn_active, player_has_control, menu_selection_index, message_label
    
    # First, properly end the player's turn and update turn counters
    end_player_turn()
    ...
    # Refill enemy RIG using RES as fuel (same system as player)
    rig_restored, res_consumed, refill_message = refill_enemy_rig_with_res()
    
    # Attempt enemy auto-regeneration multiple times if there are damaged parts
    total_regenerations = 0
    regeneration_messages = []
    if rig_restored > 0 or enemy.rig >= 5:  # Only try if RIG was refilled or already sufficient
        total_regenerations, regeneration_messages = enemy_auto_regenerate_multiple()
    
    enemy_turn_active = True
    player_has_control = False
    
    # Create comprehensive turn start message
    turn_message = f"È il turno di {enemy.name}\n{refill_message}"
    if total_regenerations > 0:
        if total_regenerations == 1:
            turn_message += f"\n{enemy.name} si è rigenerato!"
        else:
            turn_message += f"\n{enemy.name} si è rigenerato {total_regenerations} volte!"
        # Add specific regeneration messages
        for regen_msg in regeneration_messages:
            turn_message += f"\n{regen_msg}"
    elif enemy.rig < 5:
        turn_message += f"\n{enemy.name} non può rigenerarsi (RIG insufficiente)"
    elif enemy.res <= 0:
        turn_message += f"\n{enemy.name} ha esaurito le riserve!"
    
    # Display the enemy turn message
    show_message(turn_message, 3000)
    ...
    # Ensure the message stays on top after panels are created
    if message_label is not None:
        message_label.lift()
        message_label.tkraise()
    
    # Start enemy turn after a short delay
    root.after(3500, execute_enemy_turn)
...
def perform_enemy_move(move, target_part):
    ...
    if accuracy_roll > move.accuracy:
        # Move missed - play miss sound
        show_message(f"{enemy.name} ha mancato {move.name}!", 1000)
    ...
    # Move hit - apply damage and play hit sound
    damage = move.danno
    target_part.p_pvt = max(0, target_part.p_pvt - damage)
    
    # Blink player when taking damage from enemy
    blink_player()

    # Play hit sound effect based on move element
    if "CUT" in move.elem:
        # Play cutting sound effect
        play_sound_effect("mixkit-quick-knife-slice-cutting-2152.wav", volume=4)
    ...
    # Recalculate player health and update HP bars immediately
    player.calculate_health_from_body_parts()
    update_body_part_hp_bars()
    
    # Update main PVT stat bar immediately
    if 'pvt' in stat_bars:
        update_stat_bar(stat_bars['pvt'], player.pvt, player.max_pvt)
...
def end_enemy_turn_ui():
    """End the enemy turn and give control back to the player (UI function)"""
    global enemy_turn_active, player_has_control
    
    # Call the main turn ending function first to update counters and process effects
    end_enemy_turn()
    ...
    # Restore player RIG using RES as fuel (instead of direct refill)
    rig_restored, res_consumed, refill_message = refill_player_rig_with_res()
    
    enemy_turn_active = False
    player_has_control = True
    
    # Display player turn message with stamina and RIG info
    show_message(f"È il turno di {player.name}\n{enemy.name} stamina ripristinata: {enemy.sta}/{enemy.max_sta} (+{stamina_restored})\n{refill_message}", 2000)
    
    print("Enemy turn ended, control returned to player")
    print(f"Player RIG refill: restored {rig_restored}, consumed {res_consumed} RES")
    
    # Update character stats to reflect changes
    update_character_stats()
    
    # Restore proper menu navigation for the player's turn
    setup_menu_navigation()

def move_selection(event):
    global menu_selection_index, player_has_control, regeneration_mode_active
    
    # Don't allow menu navigation during enemy turn or regeneration mode
    if not player_has_control or regeneration_mode_active:
        return
...
def setup_menu_navigation():
    ...
    global regeneration_mode_active
    
    # Don't change navigation if regeneration mode is active
    if regeneration_mode_active:
        return
...
def debug_stamina_info():
    ...
    print(f"Player buf_sta: {get_character_buff_level(player, 'buf_sta')}")
    ...
    calculated_max = player.max_sta + (get_character_buff_level(player, 'buf_sta') * STAT_SCALING_FACTORS['sta'])
...
# Bind regeneration key
def on_regeneration_key(event):
    """Handle R key press for regeneration mode"""
    toggle_regeneration_mode()

root.bind("<r>", on_regeneration_key)
root.bind("<R>", on_regeneration_key)
...
# DEBUG: Add key bindings for testing turn system
def on_debug_key(event):
    """Handle debug key presses for testing turn system"""
    if event.keysym == 'F1':
        print("DEBUG: F1 pressed - Simulating player turn")
        debug_simulate_player_turn()
    elif event.keysym == 'F2':
        print("DEBUG: F2 pressed - Simulating enemy turn")
        debug_simulate_enemy_turn()
    elif event.keysym == 'F3':
        print("DEBUG: F3 pressed - Simulating full round")
        debug_simulate_full_round()
    elif event.keysym == 'F4':
        print("DEBUG: F4 pressed - Resetting turn counters")
        reset_turn_counters()

# Bind debug keys
root.bind("<F1>", on_debug_key)  # Player turn
root.bind("<F2>", on_debug_key)  # Enemy turn  
root.bind("<F3>", on_debug_key)  # Full round
root.bind("<F4>", on_debug_key)  # Reset turns

print("DEBUG: Turn testing keys bound - F1: Player turn, F2: Enemy turn, F3: Full round, F4: Reset")
