...
class NPC(pygame.sprite.Sprite):
    ...
    def update(self, player, collision_tiles, tmx_data):
        ...
        if dist <= self.sight_range:
            # Decide or continue axis priority
            if self.axis_priority is None or self.axis_steps_left <= 0:
                if abs(dx) > abs(dy):
                    self.axis_priority = 'x'
                elif abs(dy) > abs(dx):
                    self.axis_priority = 'y'
                else:
                    # If exactly diagonal, keep previous or pick randomly
            ...
            for dir_name, (mx, my) in directions:
                ...
                # If all directions blocked, allow axis switch next frame
                self.axis_steps_left = 0
        else:
...
# --- Utility: Check if player is facing the center of an object (no image) they are inside ---
...
enemy_example = NPC(
    name="Scheletro_Nemico",
    image_path=r"C:\Users\franc\Desktop\Afterdeath_RPG\Overworld\characters\Scheletro_daun.png",
    pos=(spawn_position[0] + 5, spawn_position[1]),  # Example: 5px to the right of player
    speed=3,
    active=True,
    sight_range=3000,
    groups=object_group
)
print(f"[DEBUG] Enemy in object_group: {enemy_example in object_group}")
print(f"[DEBUG] object_group has {len(object_group.sprites())} sprites. NPC in group: {enemy_example in object_group.sprites()}")
