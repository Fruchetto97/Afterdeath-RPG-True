...
class PygameUI:
    ...
    def __init__(self):
        ...
        self.font_big = pygame.font.Font(str(font_path), 28)
        ...
        # EVENT LOG SYSTEM - REPLACING MESSAGE SYSTEM
        ...
        # Log window dimensions and position (same as old message box)
        self.log_x = SCREEN_WIDTH // 2 + 300
        self.log_y = SCREEN_HEIGHT - 500
        self.log_width = SCREEN_WIDTH // 2 - 175
        self.log_height = 480
    ...
    def show_enemy_panel(self):
        """Draw enemy panel when ENEMY is selected with animated GIF and background (pygame version) - FIXED"""
        ...
        if should_show_enemy_panel:
            ...
            # Calculate menu bar dimensions and position
            menu_bar_width = 760
            menu_bar_x = 310
            menu_bar_y = SCREEN_HEIGHT - 265
            
            # Define enemy panel area aligned with menu bar
            panel_x = menu_bar_x
            panel_y = 35
            panel_width = menu_bar_width
            panel_height = menu_bar_y - panel_y - 20

            # Display enemy name - AUTOMATICALLY CENTERED
            enemy_name_text = f"{enemy.name}"

            # Calculate the positions of the body parts list and attributes box
            parts_x = panel_x + panel_width + 20  # Body parts list position
            attributes_panel_x = 1450  # Enemy part attributes box position (from draw_enemy_part_attributes)

            # Calculate the center point between the two panels
            center_x = (parts_x + 350 + attributes_panel_x) // 2  # +350 for body parts list width

            # Draw the enemy name centered between the two panels
            self.draw_text(enemy_name_text, self.font_medium, WHITE, center_x, panel_y + 25, center=True)
        
            # Draw enemy background if available
            if hasattr(self, 'enemy_background') and self.enemy_background is not None:
                # Draw the processed background
                self.screen.blit(self.enemy_background, (panel_x, panel_y))
            ...
            # Calculate centered position for the GIF aligned to bottom of background
            centered_gif_x = panel_x + (panel_width - enemy_gif_width) // 2
            # Align GIF to bottom of the background panel (with small margin)
            centered_gif_y = panel_y + panel_height - enemy_gif_height
            
            # Draw the enemy GIF centered horizontally and aligned to bottom
            self.screen.blit(current_enemy_frame, (centered_gif_x, centered_gif_y))
            
            # FIXED: Draw enemy blink overlay to cover the entire background area instead of just the GIF
            ...
            if hasattr(self, 'target_selection_active') and self.target_selection_active and hasattr(self, 'selected_move') and self.selected_move:
                ...
                # Move information - compact display
                move_info_text = f"MOSSA: {move.name} | DMG: {move.danno} | STA: {move.stamina_cost} | ACC: {move.accuracy}%"
                self.draw_text(move_info_text, self.font_medium, WHITE, panel_x + 10, move_info_y)
                
                if effects_display:
                    effects_text = f"EFFETTI: {', '.join(effects_display)}"
                    self.draw_text(effects_text, self.font_medium, WHITE, panel_x + 10, move_info_y + 25)
                
            # Enemy body parts list positioned to the right of the main panel
            parts_x = panel_x + panel_width + 20
            parts_y = panel_y + 50
            
            # Semi-transparent background for body parts list
            parts_bg_height = len(enemy.body_parts) * 40 + 20
            parts_bg_rect = pygame.Rect(parts_x - 10, parts_y - 10, 200, parts_bg_height)
            ...
            # Body parts list with enhanced highlighting for target selection
            for i, part in enumerate(enemy.body_parts):
                ...
                # Draw part name with appropriate color
                ...
                # Health fill - red tint during target selection
                if part.max_p_pvt > 0:
                    fill_width = int(bar_width * part.p_pvt / part.max_p_pvt)
                ...
                self.draw_text(hp_text, self.font_small, hp_text_color, bar_x + bar_width//2, y+8, center=True)
        else:
            print(f"DEBUG: NOT showing enemy panel - conditions not met")

    def draw_turn_display(self):
        """Draw the turn display panel at the top of the body panel"""
        # Turn display dimensions and position
        turn_x = 10
        turn_y = 10
        turn_width = 280
        turn_height = 40
        
        # Draw turn display background
        ...
        # Draw turn text centered in the display
        self.draw_text(
            turn_text, 
            self.font_medium, 
            YELLOW, 
    ...
    def draw_body_panel(self):
        """Draw the left body panel with HP bars and animated player image at bottom (updated for generalized status indicators)"""
        panel_x = 10
        panel_y = 10
        panel_width = 280
        ...
        # Calculate space needed for body parts (increased height to accommodate multiple status indicators)
        body_parts_count = len(player.body_parts)
        part_height = 90  # Increased from 75 to accommodate multiple status rows
        total_parts_height = body_parts_count * part_height + 30  # +30 for top padding
        
        # Calculate remaining space for player image
        available_image_height = panel_height - total_parts_height - 20  # -20 for bottom padding
        ...
        max_img_width = panel_width - 30  # Leave 15px margin on each side
        max_img_height = max(100, available_image_height)  # Minimum 100px height
        ...
        # Draw body parts with regeneration highlighting and generalized status indicators
        y_offset = 15
        for idx, part in enumerate(player.body_parts):
            ...
            # Part name with appropriate color
            self.draw_text(part.name, self.font_large, part_name_color, panel_x + panel_width // 2, panel_y + y_offset + 10, center=True)
            ...
            # Background bar
            pygame.draw.rect(self.screen, DARK_GRAY, (panel_x + 15, bar_y, bar_width, bar_height))
            
            # Health fill
            if part.max_p_pvt > 0:
                fill_width = int(bar_width * part.p_pvt / part.max_p_pvt)
                pygame.draw.rect(self.screen, GREEN, (panel_x + 15, bar_y, fill_width, bar_height))
    ...
    def draw_stats_panel(self):
        """Draw the stats panel with character stats"""
        stats_x = 310
        stats_y = 840
        stats_width = 760
        stats_height = 140
        ...
        player_section_x = stats_x + 10
        player_section_y = stats_y + 10
        
        # Player name
        self.draw_text(f"{player.name} STATS", self.font_medium, WHITE, player_section_x - 25 + stats_width/2, player_section_y+10, center=True)

        # Player stats in columns
        col1_x = player_section_x
        col2_x = player_section_x + 400
        col3_x = col2_x + 100
        col4_x = col3_x + 50
        stat_y = player_section_y + 30
        ...
        bar_width = 180
        bar_height = 12
        bar_offset_x = 50  # Space between text and bar
        
        # Column 1: HPS, REG, RES with health bars
        # HPS (Health Points)
        hps_text = f"HPS: {int(player.pvt)}/{int(player.max_pvt)}"
        self.draw_text(hps_text, self.font_small, WHITE, col1_x, stat_y)

        # HPS health bar
        hps_bar_x = col1_x + bar_offset_x + 80
        hps_bar_y = stat_y + 2
        ...
        self.draw_text(reg_text, self.font_small, WHITE, col1_x, stat_y + 25)
        
        # REG health bar
        reg_bar_x = col1_x + bar_offset_x + 80
        reg_bar_y = stat_y + 25 + 2
        ...
        # RES (Resistance/Resources)
        res_text = f"RES: {int(player.res)}/{int(player.max_res)}"
        self.draw_text(res_text, self.font_small, WHITE, col1_x, stat_y + 50)
    ...
    def draw_menu_bar(self):
        ...
        for i, label in enumerate(self.menu_labels):
            ...
            if i == self.menu_selection_index:
                ...
                pygame.draw.rect(self.screen, DARK_GRAY, (item_x + 5, menu_y + 5, item_width - 10, menu_height - 10))
                color = YELLOW
            else:
                color = WHITE
    ...
    def draw_moves_panel(self):
        """Draw moves panel when MOVES is selected"""
    ...
    def draw_pass_panel(self):
        """Draw pass panel when PASS is selected"""
        if self.menu_selection_index == 5:
            ...
            self.draw_text("YES", self.font_large, yes_color, pass_x, pass_y + 60)
            
            # NO option  
            no_color = RED if getattr(self, 'pass_selection', 0) == 1 else WHITE
            self.draw_text("NO", self.font_large, no_color, pass_x, pass_y + 120)
