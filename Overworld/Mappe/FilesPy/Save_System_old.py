...
class SaveSystem:
    ...
    def auto_save(self, player_stats, world_data=None, game_start_time=None):
        """Auto-save game by overwriting the main save file"""
        print(f"[SaveSystem] Auto-saving game (updating main save file)")
        # Auto-save is now just a call to create_save_file which will overwrite the existing save
    ...
    def save_game(self, save_data, character_name=None):
        """Load save data for moves menu - prioritizes main save file to get preserved custom moves"""
        try:
            # First try main save file (where custom moves are preserved)
            existing_save = self.find_existing_save(character_name)
            if existing_save:
                print(f"[SaveSystem] Loading main save file for moves: {character_name}")
                save_data = self.load_save_file(existing_save)
                if save_data and 'player' in save_data:
                    custom_moves = save_data['player'].get('custom_moves', [])
                    print(f"[SaveSystem] Found {len(custom_moves)} custom moves in main save")
                    return save_data
            
            # Fall back to autosave if main save doesn't exist
            autosave_path = self.saves_dir / f"autosave_{character_name}.json"
            if autosave_path.exists():
                print(f"[SaveSystem] Falling back to autosave for moves: {character_name}")
                save_data = self.load_save_file(str(autosave_path))
                if save_data and 'player' in save_data:
                    custom_moves = save_data['player'].get('custom_moves', [])
                    print(f"[SaveSystem] Found {len(custom_moves)} custom moves in autosave")
                    return save_data
            ...
            print(f"[SaveSystem] Error loading save for moves for {character_name}: {e}")
            return None
